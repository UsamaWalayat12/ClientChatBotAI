...
# System Prompts
LAWYER_SYSTEM_PROMPT = """You are an expert Pakistani Legal AI Assistant. 
Base your answers strictly on the provided legal contexts. 
If the information is not in the context, clearly state that you don't have that specific information."""

CLIENT_SYSTEM_PROMPT = """You are a legal assistant for clients in Pakistan. 
Base your answers strictly on the provided legal contexts but keep them simple and concise.
You MUST provide the answer using ONLY two headings:
### Short Answer
(A brief 1-2 sentence direct response)

### Overview
(A slightly more detailed explanation in simple terms)

Do NOT use complex legal jargon or provide lengthy detailed analysis unless absolutely necessary."""
...
async def chat_customer(request: ChatRequest):
    """
    Send a legal query from a client and get a simplified AI-generated answer
    """
    try:
        from chroma_test import retrieve_and_filter, call_gemini_chat, add_to_history, load_history
        import uuid
        
        # 1. Use client-side table
        TABLE_NAME = "client_chat_messages"
        
        # 2. Get user and conversation ID
        user_id = request.user_id or "default"
        conversation_id = request.conversation_id
        ...
        context_docs = retrieve_and_filter(request.query)
        
        # 4. Load history from client table
        ...
        if history:
            from chroma_test import format_history_for_prompt
            full_prompt += format_history_for_prompt(history)
            
        full_prompt += f"\nUSER QUESTION: {request.query}\nYOUR RESPONSE (Short Answer & Overview):"
        
        # 6. Call Gemini
        answer = call_gemini_chat(full_prompt)
        
        # 7. Add to history in client table
        add_to_history(history, "user", request.query, user_id=user_id, conversation_id=conversation_id, table_name=TABLE_NAME)
        add_to_history(history, "assistant", answer, user_id=user_id, conversation_id=conversation_id, table_name=TABLE_NAME)
...
async def get_customer_history(user_id: str = "default", conversation_id: Optional[str] = None):
    """Retrieve chat history for a client"""
    try:
        from chroma_test import load_history
        history = load_history(user_id=user_id, conversation_id=conversation_id, table_name="client_chat_messages")
        return {
            "success": True,
            "history": history
        }
...
@app.get("/api/customer/conversations")
async def get_customer_conversations(user_id: str = "default"):
    """
    Get all separate client chat conversations for a specific user
    """
    try:
        from chroma_test import SUPABASE_AVAILABLE
        if not SUPABASE_AVAILABLE:
        ...
        from chroma_test import supabase
        query = supabase.table('client_chat_messages').select('*').eq('user_id', user_id).order('created_at')
        
        print(f"[Client API] Fetching client conversations for user: {user_id}")
        response = query.execute()
        ...
        print(f"[Client API] Error grouping conversations: {e}")
        raise HTTPException(status_code=500, detail=str(e))

# Clear history endpoint
@app.delete("/api/history/clear")
async def clear_history(user_id: Optional[str] = "default", conversation_id: Optional[str] = None, is_client: bool = False):
    """Clear chat history for a user or specific conversation"""
    try:
        from chroma_test import clear_history as supabase_clear_history
        table_name = "client_chat_messages" if is_client else "chat_messages"
        supabase_clear_history(user_id, conversation_id, table_name=table_name)
        
        return {
            "success": True,
            "message": f"{'Client' if is_client else 'Lawyer'} chat history cleared successfully"
        }
...
@app.delete("/api/customer/history/clear")
async def clear_customer_history(user_id: str = "default", conversation_id: Optional[str] = None):
    """Clear client chat history"""
    return await clear_history(user_id=user_id, conversation_id=conversation_id, is_client=True)
